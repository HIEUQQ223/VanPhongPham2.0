...
import 'package:vanphongpham/view/product/widgets/DoiMatKhau.dart';
import 'package:vanphongpham/view/product/widgets/thongtincainhan.dart';
...
import 'widgets/Thongtinhienthi.dart';
import 'widgets/feature_shortcuts.dart';
import 'widgets/TrangThaiDonHang.dart';
import 'widgets/TaiKhoanMenu.dart';
...
class _TaiKhoanState extends State<TaiKhoan> with TickerProviderStateMixin {
  ...
  Future<void> _refreshUserInfo() async {
    final userId = _userInfo?['maTaiKhoan']?.toString();
    if (userId != null) {
      final refreshedUser = await NguoiDungService.getUserInfo(userId);
      print(
        "DEBUG - Refreshed user in _refreshUserInfo: ${refreshedUser?.toJson()}",
      );
      if (refreshedUser != null) {
        setState(() {
          _userInfo = refreshedUser.toJson();
        });
      }
    }
  }
  ...
  Future<void> _logout() async {
    ...
    if (result == true) {
      ...
      DangXuat.showLogoutDialog(context);
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => DangNhap()),
      );
          ...
          child: Column(
            children: [
              ...
              TaiKhoanMenu(
                onProfileTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) => ThongTinCaNhanView(
                      userInfo: _userInfo,
                      onRefreshData: () async {
                        // Làm mới _userInfo từ server
                        final userId = _userInfo?['maTaiKhoan']?.toString();
                        if (userId != null) {
                          final refreshedUser =
                              await NguoiDungService.getUserInfo(userId);
                          print(
                            "DEBUG - Refreshed user in onRefreshData: ${refreshedUser?.toJson()}",
                          );
                          if (refreshedUser != null) {
                            setState(() {
                              _userInfo = refreshedUser.toJson();
                ...
                onChangePasswordTap: () {
                  // final userId = _userInfo?['maTaiKhoan'];
                  // Navigator.push(
                  //   context,
                  //   MaterialPageRoute(
                  //     builder: (context) => ChangePasswordScreen(
                  //       maTaiKhoan: int.parse(userId.toString()),
