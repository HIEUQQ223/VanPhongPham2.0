...
class ChiTietDanhMucScreen extends StatelessWidget {
  final String danhMucId;

  const ChiTietDanhMucScreen({Key? key, required this.danhMucId})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: const Center(child: Text('Chi tiết danh mục đang phát triển')),
    );
  }
}

class DanhMucScreen extends StatefulWidget {
  const DanhMucScreen({Key? key}) : super(key: key);
...
class _DanhMucScreenState extends State<DanhMucScreen> {
  ...
  bool isLoading = true;
  String? errorMessage;
  bool isGridView = true; // Thêm biến trạng thái chế độ xem
      ...
      child: InkWell(
        onTap: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  ChiTietDanhMucScreen(danhMucId: danhMuc.tenDanhMuc),
            ),
          );
        },
